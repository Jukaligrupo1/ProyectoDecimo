...
.controller('SeguroIniciarCtrl', ['$scope','modalInstance', 'Restangular', 'toastr', '$filter', '$state', 'rootScope', ($scope, modalInstance, Restangular, toastr, $filter, $state, rootScope, )->
	...
	$scope.ok = ()->
		...
		Restangular.all('examenes_respuesta/iniciar').customPOST(inscripcion).then((r)->
			...
			$state.transitionTo 'panel.examenes_respuesta'
